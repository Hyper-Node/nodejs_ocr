...
var ocropus_handler = require('./ocropus_handler.js');
...
var options = { //Add lstm add tesseract docs 
    ...
    printParameters: false, //tesseract option to print parameters
    printStdout: true,      //print std output of the used command 
    printStderr: true       //print std error of the used coommand
};



function parseFile(fileOfPicture,outname,outputdir,failed,callbackDone){
    try{
            ...
            if(err) {
                ...
                fs.writeFile(outname, text, function(err){
                    if(err){
                        ...
                        //TODO consider promises for queueing this
                        //Get error rate by ocropy 
                       // ocropus_handler.check_error_rate(outputdir,outname);
                        ocropus_handler.check_confidence_rate(outputdir,outname);
                    }
                    callbackDone();
                });                
            }
