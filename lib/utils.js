...
/**
 * Filters the defined extension from a list of files 
 * @param {array} files - list of filenames, usually a directories' content
 * @param {string} extension - the extension which should be filteres out, example 'txt'
 * @returns {array} params with extension
 */
function filterFileExtension(files,extension){
    var elmRegex = new RegExp(".*\.("+extension+")");
    elmRegex.ignoreCase = true;
    elmRegex.global = true; 

    var fileslist = files.filter( function( elm ) {
        if(elm.match(elmRegex)){
            return ""
        }else{
            return elm;
        }
